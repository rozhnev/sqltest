...
<div class="container800-header">
    <h1>Teste seus conhecimentos de SQL!</h1>
</div>
...
<div class="container800-section">
    ...
    <p>São alocadas três horas para a realização do teste. Ao final do tempo (ou antes) você poderá obter uma das classificações em SQL:</p>
...
<div class="container800-section">
    ...
    <p>Resolver uma tarefa com sucesso na primeira tentativa traz pontos adicionais, e um grande número de tentativas em uma tarefa pode levar a uma nota menor.</p>
    ...
    {if $User->logged()}
        {if isset($LastTest)}
            ...
            {else}
                {* Continue open test *}
                <div style="text-align: center; margin: 36px;">
                    <a style="display:inline-block;width:240px; color: white;" href="/{$Lang}/test/{$LastTest.id}" title="Continuar teste" class="button green">Continuar teste</a>
                </div>
            {/if}
        {else}
            <h2>Boa sorte!</h2>
