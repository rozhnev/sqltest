<style>
            .banner-container {
                display: flex;
                width: 100%;
                margin: 1em 0;
            }

            .banner-sidebar {
                width: 5px; /* Set the width of the sidebar */
                background-color: #61AEE6;
                color: white;
                border-radius: 4px 0px 0px 4px;
            }

            .banner-content {
                flex: 1; /* Takes up the rest of the space */
                background-color: #293A4A;
                padding: 10px;
                /* font-family: 'Tahoma'; */
                font-size: 0.8em;
                color: white;
                line-height: 1.5;
                border-radius: 0px 4px 4px 0px;
            }
            .banner-content::first-line {
                color: #63A9E4; /* Color for the first line only */
                font-weight: bold;
            }
            .viewchannel {
                border-top: 1px solid #61AEE6;
                color: #6FB1E3;
                text-align: center;
                padding-top: 2%;
                margin-top: 7px;
            }
            .top-right-image {
                float: right;
                margin-left: 10px;
                width: 80px;
                height: auto;
                border-radius: 4px 4px 4px 4px;
            }
        </style>
        <a href="https://t.me/db_developers_channel" target="_blank" style="text-decoration: none;">
            <div class="banner-container">
                <div class="banner-sidebar"></div>
                <div class="banner-content">
                    <img src="/images/5199833295595106305.jpg" alt="DB developers channel" class="top-right-image" class="top-right-img">
                    Telegram<br><b>DB developers channel</b><br>
                    <p style="font-weight: 100; margin: 5px 0 0 0;">
                        💡 Canal para desenvolvedores de bases de dados: Oracle, PostgreSQL<br>
                        📌 Tarefas, código, melhores práticas, arquitetura, otimização<br>
                        🔄 Junte-se à comunidade — cresça connosco!
                    </p>
                    {* <p>#SQL #Oracle #PostgreSQL #PL/SQL #PL/pgSQL #DB</p>
                    
                    <div class="viewchannel">VIEW CHANNEL</div> *}
                </div>
            </div>
        </a>