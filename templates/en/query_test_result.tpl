{if $QueryTestResult.ok}
    ...
    line-height: 1.5em;">
        {if $QueryTestResult.cost > 0}
            <div style="flex: 2 1;">
            The cost of executing your query is <span style="color: {if $QueryBestCost < $QueryTestResult.cost} #E60000;{else}#11926E;{/if} font-weight: bold;">{$QueryTestResult.cost}</span> <span style="font-size: small;">(the lower the cost, the more effective the query)</span>
            {if $QueryBestCost}
                <br>Cost of the best solution: <span style="color: #11926E; font-weight: bold;">{$QueryBestCost}</span>
                {if $QueryBestCost == $QueryTestResult.cost}
                    <p style="display: flex; color: #0069E6;">
                        ...
                        Congratulations! Your request is among the best on our website!
                    </p>
                ...
                {else}
                    <p style="color: #E60000;">
                        Unfortunately, your result is a little low of the record. You have something to work on!
                    </p>
                {/if}
            {/if}
            </div>
    ...
    {else}
        <div class="question-rate-panel">
            ...
            <div class="buttons">
                ...
                <input type="radio" id="rate2" name="question_rate" value="Simple" onChange="rateQuestion({$QuestionID}, 2)"><label for="rate1">Simple</label>
                ...
                <input type="radio" id="rate4" name="question_rate" value="Difficult" onChange="rateQuestion({$QuestionID}, 4)"><label for="rate1">Difficult</label>
                <input type="radio" id="rate5" name="question_rate" value="Very hard" onChange="rateQuestion({$QuestionID}, 5)"><label for="rate1">Very hard</label>
            </div>
        </div>
    {/if}
