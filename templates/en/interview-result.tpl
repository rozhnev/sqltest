...
<section class="interview-score">
    <h2 style="margin-top: 0;">Interview result</h2>
    ...
    {foreach $InterviewResult.topics as $topic}
        <div class="interview-topic {if $topic.weak}weak{/if}">
            <div class="interview-topic-head">
                <span>{if $topic.key === 'soft_skills'}Professional skills{elseif $topic.key === 'other'}Other{else}{$topic.title|escape}{/if}</span>
            ...
            {if $topic.lessons}
                <ul>
                    {foreach $topic.lessons as $lesson}
                        <li>Worth revisiting: <a href="/{$Lang}/lesson/{$lesson.module_slug|escape:'url'}/{$lesson.lesson_slug|escape:'url'}">{$lesson.title|escape}</a></li>
                    {/foreach}
                </ul>
            {/if}
        </div>
                ...
                <ul>
                    {foreach $item.options as $option}
                        <li class="{if $option.is_valid}option-valid{/if}">
                            {if $option.selected}☑{else}☐{/if} {$option.answer}{if $option.is_valid} &mdash; correct option{/if}
                        </li>
                    {/foreach}
                </ul>
            {else}
...
<p style="text-align: center;">
    <a class="button blue" href="/{$Lang}/interview-start">Back to Meridian Logistics openings</a>
</p>
