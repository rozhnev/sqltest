{if $QueryTestResult.ok}
    ...
    <div style="display: flex; flex-flow: row; flex-wrap: wrap; line-height: 1.5em;">
        {if $QueryTestResult.cost > 0}
            <div style="flex: 2 1;">
            执行查询的成本是 {$QueryTestResult.cost} <span style="font-size: small;">（成本越低，查询越有效）</span>
            {if $QueryBestCost}
                <br>最佳解决方案的成本：{$QueryBestCost}<br>
                {if $QueryBestCost == $QueryTestResult.cost} 恭喜！您的请求在我们网站上名列前茅！
                {elseif $QueryBestCost > $QueryTestResult.cost} 恭喜您改善了我们的记录！
                {else} 不幸的是，您的结果稍低于记录。您还有改进的空间！ {/if}
            {/if}
            </div>
        {/if}
...
{elseif array_key_exists('hints', $QueryTestResult) && array_key_exists('maxAttemptsReached', $QueryTestResult.hints)}
    ...
    {translate}最大尝试次数已达到{/translate}
    {if $QueryTestResult.nextQuestion} {translate}前往下一个任务{/translate}{/if}
{elseif array_key_exists('hints', $QueryTestResult) && array_key_exists('timeOut', $QueryTestResult.hints)}
    {translate}测试超时{/translate} {translate}前往评分{/translate}
{else}
    ...
    <div class="qts-error-card" role="status" aria-live="polite">
        ...
        <div class="qts-error-body">
            ...
            {/if}
            <div class="qts-error-cta">继续努力——您快到了。</div>
