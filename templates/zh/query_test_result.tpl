{if $QueryTestResult.ok}
    ...
    line-height: 1.5em;">
        {if $QueryTestResult.cost > 0}
            <div style="flex: 2 1;">
            执行你的查询成本是 <span style="color: {if $QueryBestCost < $QueryTestResult.cost} #E60000;{else}#11926E;{/if} font-weight: bold;">{$QueryTestResult.cost}</span> <span style="font-size: small;">（成本越低，查询越高效）</span>
            {if $QueryBestCost}
                <br>最佳方案成本： <span style="color: #11926E; font-weight: bold;">{$QueryBestCost}</span>
    ...
    {else}
        <div class="question-rate-panel">
            <div style="min-width:280px;">开始下一题前，请为本题难度评分：</div>
            <div class="buttons">
     ...
     {if array_key_exists('hints', $QueryTestResult) }
        {if array_key_exists('wrongQuery', $QueryTestResult.hints) }
            <p>
                ...
                {else}。
                {/if}
                <span class="text-button blue" id="getHelpBtn" onclick="getHelp('zh', {$QuestionID})">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
