{if $QueryTestResult.ok}
    ...
    line-height: 1.5em;">
        {if $QueryTestResult.cost > 0}
            <div style="flex: 2 1;">
            执行你的查询的成本是 <span style="color: {if $QueryBestCost < $QueryTestResult.cost} #E60000;{else}#11926E;{/if} font-weight: bold;">{$QueryTestResult.cost}</span> <span style="font-size: small;">(成本越低，查询越有效)</span>
            {if $QueryBestCost}
                <br>最佳解决方案的成本： <span style="color: #11926E; font-weight: bold;">{$QueryBestCost}</span>
        ...
        {/if}
        <div>
            <button class="button green" onClick="showOthersSolutions({$QuestionID})">给我看看其他解决方案！</button>
        </div>
     </div>
     {if !$User->logged()}
        <p class="question-action">
            为了保存你的进度并能够查看其他解决方案，请 <a href="" onClick="toggleLoginWindow(); return false;">登录</a>
        </p>
    {else}
        <div class="question-rate-panel">
            <div style="min-width:280px;">在开始下一个测试之前，请评估这个任务的难度：</div>
            <div class="buttons">
...
{else}
     ...
     {if array_key_exists('hints', $QueryTestResult) }
        {if array_key_exists('wrongQuery', $QueryTestResult.hints) }
            <p>
                查询的结果是正确的，但查询本身不符合任务要求{if array_key_exists('wrongQueryHints', $QueryTestResult.hints)}:
                    <ul>
