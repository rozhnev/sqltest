{if $QueryTestResult.ok}
    {assign var="successPhrases" value=[
        "Requête brillante !",
        "Excellent travail — celle-ci est parfaite !",
        "Vous assurez, continuez comme ça !",
        "C'est un succès !",
    ...
    <div class="qts-celebration">
        ...
        <div class="qts-badge">Mission : accomplie</div>
    ...
    <div style="display: flex; flex-flow: row; flex-wrap: wrap; line-height: 1.5em;">
        {if $QueryTestResult.cost > 0}
            <div style="flex: 2 1;">
            Le coût d'exécution de votre requête est de {$QueryTestResult.cost} <span style="font-size: small;">(plus le coût est bas, plus la requête est efficace)</span>
            {if $QueryBestCost}
                <br>Coût de la meilleure solution : {$QueryBestCost}<br>
                {if $QueryBestCost == $QueryTestResult.cost} Félicitations ! Votre requête figure parmi les meilleures de notre site !
                {elseif $QueryBestCost > $QueryTestResult.cost} Félicitations pour avoir amélioré notre record !
                {else} Malheureusement, votre résultat est un peu en dessous du record. Vous avez de quoi progresser ! {/if}
            {/if}
            </div>
        {/if}
        {if array_key_exists('answerResult', $QueryTestResult)}
            <p>La réponse envoyée sera évaluée par le jury.</p>
        {/if}
    </div>
...
{else}
    {assign var="errorPhrases" value=[
        "Réessayez — un petit réglage, une grande victoire !",
        "Si près ! Peaufinez cela et c’est à vous.",
        "On y est presque — tentez encore votre chance !",
        "Un petit ajustement et vous toucherez le mille.",
        "Bel effort — relancez avec un minuscule changement."
    ]}
    ...
    <div class="qts-error-card" role="status" aria-live="polite">
        ...
        <div class="qts-error-body">
            ...
            {/if}
            <div class="qts-error-cta">Continuez — vous y êtes presque.</div>
