...
{elseif $FreeAnswerResult.ok}
    {assign var="phrases" value=[
        ...
        ['¡Increíble! Has terminado la tarea!', 'Para mantener tu progreso seguro, <a href="" onClick="toggleLoginWindow(); return false;">simplemente inicia sesión ahora</a>.'],
        ...
        ['¡Eres genial! ¡Has terminado!', 'No olvides <a href="" onClick="toggleLoginWindow(); return false;">iniciar sesión</a> para mantener todo tu progreso seguro y a salvo. 😎']
    ] }
    <p>{$phrases[$phrase_id][0]}</p>
    {if $FreeAnswerResult.comment}
        <p>{$FreeAnswerResult.comment|escape:"html"}</p>
    {/if}
    {if !$User->logged()}
...
{else}
    ...
    {$phrases[$phrase_id][0]}
    {if $FreeAnswerResult.comment}
        <p class="question-action">{$FreeAnswerResult.comment|escape:"html"}</p>
    {/if}
