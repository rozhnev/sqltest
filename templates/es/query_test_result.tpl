{if $QueryTestResult.ok}
    ...
    line-height: 1.5em;">
        {if $QueryTestResult.cost > 0}
            <div style="flex: 2 1;">
            El costo de ejecutar tu consulta es <span style="color: {if $QueryBestCost < $QueryTestResult.cost} #E60000;{else}#11926E;{/if} font-weight: bold;">{$QueryTestResult.cost}</span> <span style="font-size: small;">(cuanto menor sea el costo, más efectiva es la consulta)</span>
            {if $QueryBestCost}
                <br>Costo de la mejor solución: <span style="color: #11926E; font-weight: bold;">{$QueryBestCost}</span>
                {if $QueryBestCost == $QueryTestResult.cost}
                    <p style="display: flex; color: #0069E6;">
                        ...
                        ¡Felicidades! ¡Tu solicitud está entre las mejores de nuestro sitio web!
                    </p>
                ...
                {else}
                    <p style="color: #E60000;">
                        Desafortunadamente, tu resultado está un poco por debajo del récord. ¡Tienes algo en lo que trabajar!
                    </p>
                {/if}
            {/if}
            </div>
    ...
    {else}
        <div class="question-rate-panel">
            <div style="min-width:280px;">Antes de comenzar la siguiente prueba, por favor califica la dificultad de esta tarea:</div>
            <div class="buttons">
