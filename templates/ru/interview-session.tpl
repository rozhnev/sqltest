...
{/capture}
{capture name="elena_avatar"}<img class="dialog-avatar" src="/images/interview/meridian-logistics-representative.jpeg" alt="Елена Чо">{/capture}
...
<div class="interview-session-box lesson-wrapper">
    {if $status === 'intro' || $status === 'intro_followup' || $status === 'in_progress'}
                ...
                {if $followupQuestion}
                    <div class="dialog-row candidate">
                        ...
                        <div class="dialog-message">
                            ...
                            {if $status === 'intro_followup'}
                                <form method="post" class="dialog-bubble">
                                    <textarea name="followup_answer" required maxlength="4000" aria-label="Ответ на вопрос Елены" placeholder="Ваш ответ..."></textarea>
                                    {if $SelfIntroError}
                                        <p class="error-text">{$SelfIntroError|escape}</p>
                                    {/if}
                                    <div class="dialog-actions">
                    ...
                    {else}
                        <div class="dialog-row">
                            ...
                            <div class="dialog-message">
                                <p class="dialog-author">Елена Чо</p>
                                <div class="dialog-bubble"><p>Спасибо, это помогает лучше понять ваш опыт. Теперь давайте перейдём к вопросам.</p></div>
                            </div>
                        </div>
                    {/if}
                {/if}
        ...
        {/if}
    {else}
        <h2>Статус сессии: {$status|escape}</h2>
