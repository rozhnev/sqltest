...
- `HAVING` фильтрует строки после агрегации. Чаще всего оператор `HAVING` используют с агрегатными функциями, но  также может применятся к неагрегированным столбцам (обычно по столбцам из `GROUP BY`).
...
- Используйте `HAVING` для фильтрации строк после агрегации.
- `HAVING` работает с агрегированными данными, а `WHERE` — нет.
- Дополняйте `GROUP BY` предложением `HAVING` для расширения возможностей анализа и отчетности.
