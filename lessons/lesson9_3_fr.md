...
Le nom d’une table temporaire ne doit pas coïncider avec celui d’une table permanente existante ; il est donc recommandé d’utiliser un préfixe comme `temp_`.
...
*Résultat : la première requête renvoie la liste des clients actifs, et la seconde calcule des indicateurs de synthèse sur le même jeu de données intermédiaire. C’est l’un des principaux avantages d’une table temporaire : contrairement à une sous-requête ou à un CTE, elle peut être réutilisée dans plusieurs requêtes distinctes sans réécrire la même logique.*
...
- **Une table classique** participe généralement au stockage durable et est destinée à conserver les données de façon permanente.
- **Une table temporaire** n’est généralement pas considérée comme un stockage durable : ses données ne sont utiles que pendant une session ou lors d’étapes intermédiaires de traitement et ne doivent en règle générale pas survivre à la fin du travail.
...
Dans certains cas, on peut utiliser un CTE (`WITH`) ou une sous-requête au lieu d’une table temporaire. La différence est la suivante :

- **un CTE** n’existe que dans le cadre d’une seule requête ;
- **une sous-requête** n’existe elle aussi qu’à l’intérieur de la requête où elle est écrite ;
...
- **une table temporaire** est pratique si le résultat intermédiaire doit être réutilisé dans deux requêtes distinctes ou plus.

Si le résultat n’est nécessaire qu’une seule fois, un CTE ou une sous-requête est souvent plus simple. S’il est nécessaire à plusieurs étapes, une table temporaire est généralement plus pratique, car il n’est pas nécessaire de réécrire la même requête intermédiaire à chaque étape suivante.

---

## Tables temporaires en mémoire

Dans certains SGBD, il est possible de créer explicitement une table temporaire de sorte qu’elle soit stockée en mémoire vive. Cela peut accélérer nettement l’accès aux données, surtout si la table est compacte et très utilisée dans plusieurs étapes d’un même scénario.

Par exemple, dans MySQL ou MariaDB, on peut indiquer explicitement le moteur de table :
...
Cette approche exige d’indiquer explicitement le `ENGINE` approprié dans la définition de la table. Elle doit être utilisée avec prudence : la mémoire vive est limitée, et des tables temporaires trop volumineuses peuvent créer une charge inutile sur le serveur.
...
- donnez aux tables temporaires des noms explicites qui reflètent leur rôle, et utilisez un préfixe comme `tmp_` pour les noms de tables temporaires ;
- n’attendez pas des tables temporaires le même niveau de durabilité que des tables classiques ;
- placez les tables temporaires en mémoire uniquement lorsque cela est réellement justifié par le volume de données et la charge ;
...
Cette approche est particulièrement pratique si, après cette liste, vous devez exécuter plusieurs autres requêtes analytiques. Par exemple, après avoir obtenu la liste des clients, vous pouvez immédiatement en calculer le nombre total sans répéter toute la logique de sélection initiale.

C’est précisément la différence importante avec les sous-requêtes et les CTE : une table temporaire peut être créée une seule fois, puis utilisée successivement dans plusieurs requêtes indépendantes.
...
*   Elles existent généralement uniquement dans le cadre de la session ou de la transaction en cours et sont supprimées automatiquement sauf si autre chose est précisé lors de leur création, mais en pratique il vaut mieux les supprimer explicitement.
*   Leur syntaxe et leur usage sont proches des tables classiques, mais elles ne sont pas destinées au stockage permanent.
*   Les tables temporaires ne sont généralement pas utilisées comme stockage durable au sens de `Durability`.
...
*   Dans certains SGBD, de petites tables temporaires peuvent être placées en mémoire, mais cela doit être fait avec prudence.
