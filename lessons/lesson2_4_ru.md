...
## Важная ремарка о переносимости

С областью видимости псевдонимов в SQL нужно быть осторожным. В разных СУБД правила могут заметно отличаться: одна база данных разрешает использовать алиас в одних частях запроса, а другая в тех же местах выдаст ошибку.

Это особенно важно в пограничных случаях: при работе с `HAVING`, оконными конструкциями `OVER`, вложенными подзапросами в списке `SELECT` и другими более сложными выражениями. Здесь не стоит полагаться на «общую интуицию» или привычки из другой СУБД.

Практическое правило простое: если запрос должен быть переносимым, всегда проверяйте документацию конкретной СУБД или тестируйте поведение экспериментально.
