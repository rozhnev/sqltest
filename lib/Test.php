    ...
    public function getQuestionData(int $qusestionId): array 
    {
        $stmt = $this->dbh->prepare("
            WITH question_data AS (SELECT 
                ...
                LAG(test_questions.question_id) OVER (ORDER BY categories.sequence_position, question_categories.sequence_position) previous_question_id,
                LEAD(test_questions.question_id) OVER (ORDER BY categories.sequence_position, question_categories.sequence_position) next_question_id,
